...
class VNNOccScfNetMulti(nn.Module):
    ...
    def forward(self, input):
        ...
        if self.return_features:
            ...
            out_dict['scf'], out_dict['features'] = self.decoder_scf(query_points, z)
        else:
    ...
    def forward_latent(self, z, coords, return_dict=False):
        ...
        out_dict['scf'], out_dict['features'] = self.decoder_scf(coords, z)
...
class VNNOccScfNetShared(nn.Module):
    def __init__(self,
                 latent_dim,
                 model_type='pointnet',
                 sigmoid=True,
        ...
        self.decoder = DecoderInnerOccScfShared(dim=3, z_dim=latent_dim, c_dim=0, o_dim=5, hidden_size=latent_dim,
                                                leaky=True, sigmoid=sigmoid, return_features=return_features, acts=acts)
    ...
    def forward(self, input):
        ...
        if self.return_features:
            out_dict['occ'], out_dict['scf'], out_dict['features_occ'] = self.decoder(query_points, z)
        else:
...
class DecoderInnerOccScfShared(nn.Module):
    ''' Decoder class.
