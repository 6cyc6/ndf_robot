...
class VNNOccSdfScfNetShared(nn.Module):
    def __init__(self,
                 latent_dim,
                 model_type='pointnet',
                 sigmoid=True,
        ...
        self.decoder = DecoderInnerOccSdfScfShared(dim=3, z_dim=latent_dim, c_dim=0, o_dim=5, hidden_size=latent_dim,
                                                   leaky=True, sigmoid=sigmoid, return_features=return_features, acts=acts)
    ...
    def forward(self, input):
        ...
        if self.return_features:
            out_dict['occ'], out_dict['sdf'], out_dict['scf'], out_dict['features_occ'] = self.decoder(query_points, z)
        else:
            out_dict['occ'], out_dict['sdf'], out_dict['scf'] = self.decoder(query_points, z)
    ...
    def forward_latent(self, z, coords, return_dict=False):
        out_dict = {}
        coords = coords * self.scaling
        out_dict['occ'], out_dict['sdf'], out_dict['scf'], out_dict['features'] = self.decoder(coords, z)
...
class DecoderInnerOccSdfScfShared(nn.Module):
    ''' Decoder class.
    ...
    def forward(self, p, z, c=None, **kwargs):
        ...
        if self.return_features:
            ...
            return out_occ, out_sdf, out_scf, acts
        else:
            return out_occ, out_sdf, out_scf
