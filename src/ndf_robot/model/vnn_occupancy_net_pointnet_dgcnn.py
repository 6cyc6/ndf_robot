    ...
    def forward(self, p, z, c=None, **kwargs):
        ...
        if self.return_features:
            ...
            if self.acts == 'all':
                ...
                # acts = torch.cat((acts[1], acts[2], acts[3], acts[4]), dim=-1)
                # acts = torch.cat((acts[1], acts[2], acts[3], acts[4], acts[5]), dim=-1)
                # acts = torch.cat((acts[0], acts[1], acts[2], acts[3]), dim=-1)
                # acts = torch.cat((acts[1], acts[2], acts[4]), dim=-1)
