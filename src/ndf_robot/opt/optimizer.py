...
from src.utils.config_util import load_dataclass
from eval.config.config import InferenceConfig

from ndf_robot.utils import util, torch_util, trimesh_util, torch3d_util
from ndf_robot.utils.plotly_save import plot3d

from src.reconstruction.mesh_reconstruction import MeshReconstruction

cfg = load_dataclass(InferenceConfig, '/ikun/git_project/multi-feature-implicit-model/eval/config/ndf_exp.yaml')


class OccNetOptimizer:
    def __init__(self, model, query_pts, query_pts_real_shape=None, opt_iterations=250, 
                 noise_scale=0.0, noise_decay=0.5, single_object=False, grasp=False):
    ...
    def optimize_transform_implicit(self, shape_pts_world_np, ee=True, *args, **kwargs):
        ...
        for i in range(len(self.demo_info)):
            ...
            ps.register_point_cloud("pcd", demo_shape_pts_cent_np, radius=0.005, color=[0, 0, 1], enabled=True)
            ps.register_point_cloud("q", demo_query_pts_cent_perturbed_np, radius=0.005, color=[0, 1, 0], enabled=True)
            ps.show()
        ...
        if self.shape_completion:
            ...
            recon_pts = recon_mesh.sample(5000)
